...
Often when you use an alaignment of entire chromosome to extract a collinear region to identify orthologs sometimes the ends aren't eactly perfect. You may wish to remove sequences after an element or pattern in order to not have long trailing overhangs beyond the alignment region. I made a script to do this. It is called `delete_seq_following_pattern_within_multiFASTA.py` and it can be found in my ['Sequencework/AdjustFASTA_or_FASTQ' code repository](https://github.com/fomightez/sequencework/tree/master/AdjustFASTA_or_FASTQ).
